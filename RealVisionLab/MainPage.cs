    ...
    public partial class MainPage : Form
    {
        ...
        private void Audi_uzunluk_hesapla(Bitmap bitmap, int r_n, int c_n)
        {
            try
            {
                ...
                int ust = 0;
                for (int j = bitmap.Height / 2; j < bitmap.Height; j++)
                {
                    for (int i = bitmap.Width / 3; i < bitmap.Width / 3 * 2; i++)
                    {
                        Color c = bitmap.GetPixel(i, j);
                        if ((c.R + c.G + c.B) / 3 < Thrs[r_n]) c = blk;
                        else c = wht;
                        if (c == blk) ust = Math.Max(ust, j);
                    }
                }
                Ofset[r_n, 1] = ust;
                if (Ofset[r_n, 0] <= Ofset[r_n, 1] && Ofset[r_n, 1] <= Ofset[r_n, 2]) hata[c_n, r_n] = false;
